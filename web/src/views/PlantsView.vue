...
const search = ref(''), status = ref(''), dataStatus = ref('');
...
<template>
  ...
  <template v-else>
    <form class="filters card" aria-label="Filtros de plantas" @submit.prevent>
      ...
      <label>Estado HYXi<select v-model="status"><option value="">Todos</option><option v-for="(label, key) in statuses" :key="key" :value="key">{{ label }}</option></select></label>
    ...
    <ul v-else class="plant-list">
      <li v-for="plant in filteredPlants" :key="plant.id">
        <RouterLink :to="`/plants/${plant.id}`" class="card plant-row" :aria-label="`Ver detalle de ${plant.name}`">
          <div class="plant-heading"><h2>{{ plant.name }}</h2><span class="provider">{{ plant.provider }}</span><span class="open-label">Ver detalle →</span></div>
...
<style scoped>
.filters { display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 18px; padding: 24px; }
...
@media (max-width: 720px) { .filters { grid-template-columns: 1fr; } .open-label { margin-left: 0; } }
